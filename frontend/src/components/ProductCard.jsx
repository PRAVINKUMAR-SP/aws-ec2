...
const ProductCard = ({ product, onAddToCart }) => {
    ...
    return (
        ...
        >
            ...
            {/* Dynamic Sale Badge */}
            {product.price < 100 && (
                <div className="absolute top-4 left-4 bg-red-500 text-white text-xs font-bold px-3 py-1 rounded-sm z-10 shadow-sm">
                    SALE
                </div>
            )}

            {/* Image Container */}
            <div className="relative w-full h-56 overflow-hidden bg-white flex items-center justify-center p-6">
                {product.imageUrl ? (
                    <img 
            ...
            <div className="p-5 flex flex-col flex-grow bg-white border-t border-gray-50">
                <h3 className="font-extrabold text-lg text-gray-900 mb-1 line-clamp-1">{product.title}</h3>
                ...
                <div className="flex items-center gap-1 mb-2">
                    ...
                    <span className="text-xs text-gray-400 ml-1 font-medium">(128)</span>
                </div>
                ...
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-100">
                    <div className="flex flex-col">
                        <span className="text-xs text-gray-400 line-through font-semibold">₹{(product.price * 1.25).toFixed(2)}</span>
                        <span className="text-2xl font-extrabold text-gray-900">₹{product.price.toFixed(2)}</span>
                    </div>
                    <button 
                        className="bg-accent text-white px-4 py-2 rounded-full font-bold text-sm hover:bg-emerald-600 transition-all duration-300 shadow-md hover:shadow-lg group-hover:scale-105 flex items-center gap-1.5"
                        onClick={(e) => {
                            e.stopPropagation();
                            onAddToCart();
                        }}
