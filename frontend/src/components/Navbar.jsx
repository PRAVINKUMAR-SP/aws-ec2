...
const Navbar = ({ onSearch, onReset }) => {
    ...
    return (
        <>
        <nav className="bg-gradient-to-r from-blue-900 to-indigo-950 sticky top-0 z-50 shadow-md border-b border-indigo-900">
            {/* Top Row: Hamburger + Logo + (Desktop Search) + Actions */}
            <div className="flex justify-between items-center px-3 md:px-16 py-2.5 md:py-4">
                ...
                <div className="hidden md:block flex-grow max-w-3xl mx-12 relative group" ref={searchRef}>
                    <input 
                    ...
                    {isSearchFocused && searchQuery.trim() !== '' && (
                        <div className="absolute top-full mt-2 w-full bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden z-50 max-h-80 overflow-y-auto">
                            {searchResults.length > 0 ? (
                                <div className="py-1">
                                    <h3 className="px-3 py-1.5 text-xs font-bold text-gray-400 uppercase tracking-wider">Products</h3>
                ...
                {/* Right Actions */}
                <div className="flex gap-0.5 md:gap-2 items-center flex-shrink-0 ml-auto md:ml-0">
                    {isAdmin && (
            ...
            <div className="md:hidden px-3 pb-2.5 relative" ref={searchRef}>
                <input 
                    className="w-full pl-9 pr-4 py-2 rounded-full border-0 text-sm shadow-inner bg-white/95 text-gray-800 placeholder-gray-500 focus:bg-white focus:ring-2 focus:ring-emerald-400 focus:outline-none transition-all duration-300"
                    type="text" 
                    ...
                    onFocus={() => setIsSearchFocused(true)}
                />
                <div className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" style={{ marginTop: '-5px' }}>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
                </div>
                ...
                {isSearchFocused && searchQuery.trim() !== '' && (
                    <div className="absolute left-3 right-3 top-full mt-1 bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden z-50 max-h-72 overflow-y-auto">
                        {searchResults.length > 0 ? (
                            <div className="py-1">
                                <h3 className="px-3 py-1.5 text-[10px] font-bold text-gray-400 uppercase tracking-wider">Products</h3>
                                {searchResults.map(product => (
                                    <div 
                                        key={product.id}
                                        onClick={() => handleResultClick(product.id)}
                                    ...
                                    >
                                        <div className="w-9 h-9 flex-shrink-0 bg-white border border-gray-100 rounded flex items-center justify-center p-0.5">
                                            <img src={product.imageUrl} alt={product.title} className="max-h-full max-w-full object-contain" />
                                        </div>
                                        <div className="flex-grow min-w-0">
                                            <h4 className="text-xs font-bold text-gray-900 truncate">{product.title}</h4>
                                            <p className="text-[10px] text-emerald-600 font-medium">{product.category}</p>
                                        </div>
                                        <div className="text-right flex-shrink-0">
